...
class CQLOptimizer:
    ...
    def train_from_transitions(self, transitions, epochs=100, batch_size=256, alpha=1.0):
        """
        Train CQL from pre-collected transitions
        
        Args:
            transitions: List of transition dicts with keys: 'state', 'action', 'reward', 'next_state', 'done', 'quality'
            epochs: Number of training epochs
            batch_size: Batch size for training
            alpha: CQL penalty weight
        """
        print(f">>> Starting CQL Training from {len(transitions)} transitions on {self.device}...")
        print(f"    Epochs: {epochs}, Batch Size: {batch_size}, CQL Alpha: {alpha}")
        ...
        for epoch in range(epochs):
            # Sample batch from transitions
            batch = random.sample(transitions, min(self.batch_size, len(transitions)))
            ...
            if epoch % 10 == 0:
                self.update_target()
                if epoch % 20 == 0:
                    print(f" [Epoch {epoch}/{epochs}] Loss: {total_loss.item():.4f} (Bellman: {bellman_loss.item():.4f}, CQL: {cql_loss.item():.4f})")
                    self.writer.add_scalar('Loss/Total', total_loss.item(), epoch)
                    self.writer.add_scalar('Loss/Bellman', bellman_loss.item(), epoch)
                    self.writer.add_scalar('Loss/CQL', cql_loss.item(), epoch)

        print(">>> CQL Training Complete.")
        self.writer.close()

    def save_model(self, path='cql_model.pth'):
        """Save the trained Q-network"""
        torch.save(self.q_net.state_dict(), path)
        print(f">>> CQL Model saved to '{path}'")
    
    def load_model(self, path='cql_model.pth'):
        """Load a trained Q-network"""
        if os.path.exists(path):
            self.q_net.load_state_dict(torch.load(path, map_location=self.device))
            self.target_q_net.load_state_dict(self.q_net.state_dict())
            print(f">>> CQL Model loaded from '{path}'")
        else:
