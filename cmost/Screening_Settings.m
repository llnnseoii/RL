...
handles.Colohandles = {'Colo_PercPop', 'Colo_adherence', 'Colo_y_start', 'Colo_y_end',...
     'Colo_interval', 'Colo_y_after_colo', 'Colo_specificity'};  #colonoscopy 관련 변수 이름들
...
% Rectosigmo   
function RectoSigmo_PercPop_Callback(hObject, eventdata, handles) #RL에서는 무시. 어차피 100%니까.  
...
function RectoSigmo_adherence_Callback(hObject, eventdata, handles). #RL에서는 어차피 action이면 검사하므로 무시. 
...
if succ, if num >=0, handles.Variables.Screening.Rectosigmoidoscopy(c)=num; end, end, handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>    
function RectoSigmo_y_start_Callback(hObject, eventdata, handles) #RL에서는 삭제
c=4; tmp=get(handles.(handles.RectoSigmohandles{c}), 'string'); [num, succ] =str2num(tmp); 
if succ, if num >=0, handles.Variables.Screening.Rectosigmoidoscopy(c)=num; end, end, handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>    
function RectoSigmo_y_end_Callback(hObject, eventdata, handles)  #RL에서는 삭제
c=5; tmp=get(handles.(handles.RectoSigmohandles{c}), 'string'); [num, succ] =str2num(tmp); 
if succ, if num >=0, handles.Variables.Screening.Rectosigmoidoscopy(c)=num; end, end, handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>    
function RectoSigmo_interval_Callback(hObject, eventdata, handles)  #RL에서는 삭제. PPO가 결정  
...
if succ, if num >=0, handles.Variables.Screening.Rectosigmoidoscopy(c)=num; end, end, handles = MakeImagesCurrent(hObject, handles); %#ok<NASGU>    
function RectoSigmo_specificity_Callback(hObject, eventdata, handles) #**. config에 저장해야 함. 
...
% FOBT Sensitivities    #용종 크기별 발견 확률, 암 stage별 발견 확률
