...
% INITIALIZE
n = 50000;
Included         = true(1, 50000); % initially all patients are included
Alive            = true(1, 50000); % initially all patients are alive
DeathCause       = zeros(1, 50000);
DeathYear        = zeros(1, 50000);
NaturalDeathYear = zeros(1, 50000);
...
TumorRecord.Stage         = zeros(100, round(50000/10));
TumorRecord.Location      = zeros(100, round(50000/10));
TumorRecord.Sojourn       = zeros(100, round(50000/10));
TumorRecord.DwellTime     = zeros(100, round(50000/10));
TumorRecord.Gender        = zeros(100, round(50000/10));
TumorRecord.Detection     = zeros(100, round(50000/10));
TumorRecord.PatientNumber = zeros(100, round(50000/10)); % RS ONLY !!!!!!
...
DwellTimeProgression   = zeros(100, round(50000/10));
DwellTimeFastCancer    = zeros(100, round(50000/10));

Last.Colonoscopy  = zeros(1, 50000);
Last.Polyp        = ones(1, 50000) *-100;
Last.AdvPolyp     = ones(1, 50000) *-100;
Last.Cancer       = ones(1, 50000) *-100;
Last.ScreenTest   = zeros(1,50000);
Last.Included     = zeros(1,50000); % RS ONLY !!!!!!
Last.TestDone     = zeros(1,50000); % RS ONLY !!!!!!
Last.TestYear     = zeros(1,50000); % RS ONLY !!!!!!
Last.TestYear2    = zeros(1,50000); % RS ONLY !!!!!!
AusschlussPolyp = 0;
AusschlussCa    = 0;
AusschlussKolo  = 0;
PosPolyp        = 0;
...
Polyp.Polyps            = zeros(50000, 51);   % we initialize the polyps cells
Polyp.PolypYear         = zeros(50000, 51);   % we initialize the field for keeping track of years
Polyp.PolypLocation     = zeros(50000, 51);
Polyp.AdvProgression    = zeros(50000, 51);
Polyp.EarlyProgression  = zeros(50000, 51);

Ca.Cancer               = zeros(50000, 25);    % we initialize the cancer cells
Ca.CancerYear           = zeros(50000, 25);    % we initialize the field for keeping track of years
Ca.CancerLocation       = zeros(50000, 25);
Ca.TimeStage_I          = zeros(50000, 25);
Ca.TimeStage_II         = zeros(50000, 25);
Ca.TimeStage_III        = zeros(50000, 25);
Ca.SympTime             = zeros(50000, 25);
Ca.SympStage            = zeros(50000, 25);
Ca.DwellTime      = zeros(50000, 25);

% consider increasing this to 
Detected.Cancer         = zeros(50000, 50);    % this cancers are detected, patient might die
Detected.CancerYear     = zeros(50000, 50);    % year this cancer had beed diagnosed
Detected.CancerLocation = zeros(50000, 50);
Detected.MortTime       = zeros(50000, 50);

HasCancer = zeros(100, 50000);
NumPolyps = zeros(100, 50000);
MaxPolyps = zeros(100, 50000);
...
DiagnosedCancer    = zeros(100, 50000);
NumCancer          = zeros(100, 50000);
MaxCancer          = zeros(100, 50000);
...
YearIncluded = false(100, 50000);
YearAlive    = false(100, 50000);
